#!/usr/bin/env python3
"""Build a SemEval-2017 Task 5 (microblogs + headlines) agent-prob cache
that mirrors the schema used by the Twitter Financial News and Financial
PhraseBank caches.

Mirrors ``scripts/build_phrasebank_cache.py``; the only differences are
the data loader (``DataConfig(dataset_name="semeval2017")``) and the
output directory (``results/semeval2017/``).

Outputs
-------
results/semeval2017/_cache_agents.npz
results/semeval2017/_cache_finbert.npz
results/finbert_finetuned_semeval/   -- saved HF model + tokenizer
"""
...
from mas.config import LABELS, RESULTS_DIR, DataConfig, TransformerConfig
from mas.data import load_financial_phrasebank
from mas.data.preprocessing import preprocess_batch

OUT_DIR = RESULTS_DIR / "semeval2017"
OUT_DIR.mkdir(parents=True, exist_ok=True)
FT_DIR = RESULTS_DIR / "finbert_finetuned_semeval"
...
def main() -> None:
    t_total = time.time()
    print("=" * 70)
    print("  Building SemEval-2017 Task 5 agent-prob cache")
    print("=" * 70)

    cfg = DataConfig(dataset_name="semeval2017")
    ...
    print("\n  [1/4] TF-IDF + LogReg (fit on SemEval-2017 train)")
    ...
    print("\n  [3/4] GPT-4o-mini zero-shot on SemEval-2017 val+test")
    llm = SingleLLMAgent(mode="zero_shot")
    llm_val_labels, llm_val_conf = _run_llm(
        llm, val_x, n_workers=8, cache_path=OUT_DIR / "_llm_val_cache.json"
    )
    ...
    llm_test_labels, llm_test_conf = _run_llm(
        llm2, test_x, n_workers=8, cache_path=OUT_DIR / "_llm_test_cache.json"
    )
    ...
    print("\n  [4/4] Fine-tuning FinBERT on SemEval-2017")
    ...
    targs = TrainingArguments(
        output_dir=str(RESULTS_DIR / "_finbert_semeval_trainer_scratch"),
    ...
    print(f"\n  Wrote {OUT_DIR/'_cache_agents.npz'}")
    print(f"  Wrote {OUT_DIR/'_cache_finbert.npz'}")
